...
final class InstitutionalDomainResource extends Resource
{
    ...
    public static function getNavigationLabel(): string { return __('ConferenceDiscountEligibility::messages.institutional_domains'); }
    public static function getModelLabel(): string { return __('ConferenceDiscountEligibility::messages.institutional_domain'); }
    public static function getPluralModelLabel(): string { return __('ConferenceDiscountEligibility::messages.institutional_domains'); }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            Tables\Columns\TextColumn::make('normalized_domain')->label(__('ConferenceDiscountEligibility::messages.domain'))->searchable()->copyable(),
            ...
            Tables\Columns\IconColumn::make('include_subdomains')->label(__('ConferenceDiscountEligibility::messages.include_subdomains'))->boolean(),
            Tables\Columns\TextColumn::make('identity_policy')
                ...
                ->formatStateUsing(static function ($state): string {
                    $policy = DomainIdentityPolicy::tryFrom((string) $state)
                        ?? DomainIdentityPolicy::VerifiedEmailOnly;

                    return $policy === DomainIdentityPolicy::VerifiedEmailOnly
                        ? __('ConferenceDiscountEligibility::messages.domain_policy_verified_only_short')
                        : __('ConferenceDiscountEligibility::messages.domain_policy_confirmed_author_short');
                })
                ->badge(),
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        return parent::getEloquentQuery()->where('scheduled_conference_id', $id ?? -1);
    }
    ...
    public static function getPages(): array { return ['index' => Pages\ListInstitutionalDomains::route('/'), 'create' => Pages\CreateInstitutionalDomain::route('/create'), 'edit' => Pages\EditInstitutionalDomain::route('/{record}/edit')]; }
}

...
4. Upload **`ConferenceDiscountEligibility-1.0.3.zip`** over the existing plugin.
5. Enable the plugin if the update disabled it, then reload the scheduled-conference panel.
6. Confirm version **1.0.3** in Plugin Management.
7. The boot installer upgrades schema version 1 to version 2 idempotently by adding `identity_policy` to domain rules. Existing rules remain `verified_email_only`.
...
2. Upload `ConferenceDiscountEligibility-1.0.3.zip` through **Upload Plugin**.
...
- Confirm the Payment total, invoice line, snapshot evidence, audit entry, and Discount Payment Report.

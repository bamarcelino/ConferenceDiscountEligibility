...
- Fixed the Audit Log detail page that returned HTTP 500 for JSON payloads.
- Added detailed recalculation results, including matched, discounted, unchanged, skipped, paid, failed, and unverified-domain counts.
- Recorded recalculation failures in the audit trail and Laravel log.
- Distinguished recalculations completed without an eligible discount.
- Added exact-user warnings for similar but distinct email addresses and user IDs.
- Executed recalculation toggles on rule edit forms.
- Safely rendered nested and invalid-UTF-8 audit payloads.
- No schema changes.
